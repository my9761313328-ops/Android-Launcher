...
class UninstallAppsActivity : ComponentActivity() {
    ...
    private fun startAppsScan() {
        ...
        thread {
            val result = try {
                InstalledAppsScanner.performScan(applicationContext)
            } catch (e: Exception) {
                InstalledAppsResult(apps = emptyList())
            }
    ...
    private fun syncNoAppsTextSizes() {
        val headerSubtitle = findViewById<TextView>(R.id.textUninstallSubtitle)
        headerSubtitle.post {
            val measuredSize = headerSubtitle.textSize
            findViewById<TextView>(R.id.textUninstallNoAppsTitle)
                .setTextSize(TypedValue.COMPLEX_UNIT_PX, measuredSize)
            findViewById<TextView>(R.id.textUninstallNoAppsSubtitle)
                .setTextSize(TypedValue.COMPLEX_UNIT_PX, measuredSize * 0.8f)
        }
    }

    private fun populateAppsList(result: InstalledAppsResult) {
        val contentContainer = findViewById<FrameLayout>(R.id.containerUninstallContent)
        val container = findViewById<LinearLayout>(R.id.containerUninstallApps)
        ...
        contentContainer.post {
            ...
            val uninstallButtons = mutableListOf<Button>()
            val appNameViews = mutableListOf<TextView>()
            val appPackageViews = mutableListOf<TextView>()

            result.apps.forEachIndexed { index, app ->
                val row = inflater.inflate(R.layout.item_uninstall_app, container, false)

                row.findViewById<ImageView>(R.id.imageUninstallAppIcon).setImageDrawable(app.icon)

                val nameView = row.findViewById<TextView>(R.id.textUninstallAppName)
                nameView.text = app.label
                appNameViews.add(nameView)

                val packageView = row.findViewById<TextView>(R.id.textUninstallAppPackage)
                packageView.text = app.packageName
                appPackageViews.add(packageView)

                val uninstallButton = row.findViewById<Button>(R.id.buttonUninstallApp)
                ...
                params.bottomMargin = if (index == result.apps.lastIndex) 0 else cardGap
                row.layoutParams = params

                container.addView(row)
            }

            applyResponsiveButtonTextSize(uninstallButtons, appNameViews, appPackageViews)
        }
    }
